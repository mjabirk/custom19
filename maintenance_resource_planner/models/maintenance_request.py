...
class MaintenanceRequest(models.Model):
    ...
    reference = fields.Char('Maintenance Reference', index=True, copy=False, default='New', readonly=True)
    product_line_ids = fields.One2many('maintenance.request.lines', 'maintenance_id', string="Items", copy=True)
    warehouse_id = fields.Many2one('stock.warehouse', string="Warehouse",
                                   default=lambda self: self._default_warehouse_id(), check_company=True)
    picking_ids = fields.One2many('stock.picking', 'maintenance_request_id', string='Transfers')
    purchase_ids = fields.One2many('purchase.order', 'maintenance_request_id', string='Purchase Orders')
    ...
    expense_ids = fields.One2many('hr.expense', 'maintenance_request_id', string='Expenses')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if vals.get('reference', 'New') == 'New':
                ...
                if 'date' in vals:
                    maintenance_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['schedule_date']))
                vals['reference'] = self_comp.env['ir.sequence'].next_by_code('maintenance.request', sequence_date=maintenance_date) or '/'
    ...
    def create_expense(self):
        ''' Create the expenses from maintenance.
        ...
        for line in self.product_line_ids:
            if line.procure_method == 'expense' and line.product_uom_qty > line.product_qty_ordered:
                expense = self.env['hr.expense'].create({
                    ...
                    'maintenance_request_lines_id':line.id,
                    'employee_id':self.env.user.with_company(self.env.company).employee_id.id,
                })
    ...
    def create_picking(self):
        ...
        for line in self.product_line_ids:
            if line.product_id and line.product_uom_qty > line.product_qty_ordered:
                ...
                elif line.procure_method == 'buy':
                    if not line.partner_id:
                        raise UserError(_('Please add vendor to create purchase order!'))
                    purchase_line_vals.append((line.partner_id, {
                        'name': line.name,
                        'product_qty': line.product_uom._compute_quantity(
                            line.product_uom_qty - line.product_qty_ordered, line.product_id.uom_id),
                        'product_id': line.product_id.id,
...
class MaintenanceRequestLines(models.Model):
    _name = 'maintenance.request.lines'
    _description = "Maintenance Request Lines"

    maintenance_id = fields.Many2one('maintenance.request', string='Maintenance Request', required=True, ondelete='cascade', index=True, copy=False)
    ...
    company_id = fields.Many2one(related='maintenance_id.company_id', string='Company', store=True, readonly=True,
                                 index=True)
    ...
    purchase_line_ids = fields.One2many('purchase.order.line', 'maintenance_request_lines_id', string='Purchase Order Lines')
    move_ids = fields.One2many('stock.move', 'maintenance_request_lines_id', string='Stock Moves')
    ...
    expense_ids = fields.One2many('hr.expense', 'maintenance_request_lines_id', string='Expenses')


    # TODO: disable deleting this and maintenance request if there is stock move, purchase or expense related to this
    # @api.ondelete(at_uninstall=True)
    # def _unlink_if_no_record(self):
